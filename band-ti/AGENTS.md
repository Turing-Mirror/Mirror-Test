...
- The primary flow is a 20-question personality quiz that returns one character from the local 72-character girl-band-anime catalog.
- Use the downloaded local official character art from `public/assets/characters/`; do not replace it with generated art.
- Maintain a 150-question, 10-category bank and draw exactly two questions from each category for every 20-question session. Keep the full 72-character roster rotating on the home page, link each character to its supplied Moegirl Wiki entry, and show the Turing Mirror QQ group (1077458748) plus its QR code in result sharing.
...
- 2026-09-02：产品文案只表达用户可执行的功能或内容本身；禁止加入无需找链接、结果已保存等解释开发意图、自我说明或与功能无关的措辞。
- 2026-09-02：每局固定随机题序与选项序，返回上一题时保留同一顺序。结果使用独立会话随机种子和经过题库校正的特质加权抽取；72 名角色在随机答题样本中的边际命中率必须近似一致，并保持自动化分布校验。
